...
- [ ] 1.1 Create `game.js` with the level text from §3 as a module constant in canonical
...
- [ ] 1.2 Verify the export block is inert in a browser: confirm it is guarded on `module`
      being defined, and that loading the file by a plain script tag raises no error and
      defines no unexpected global.
- [ ] 1.3 Create `test.js` using only `node:test` and `node:assert`, containing an
...
- [ ] 2.1 Implement `parseLevel`: split trimmed text into rows and build state with `walls`
...
- [ ] 2.2 Extend parsing to accept `X` and `b`. Verify a board containing `X` yields a cell
      holding both a pad and a crate, and one containing `b` yields a cell holding both a
      pad and the bot.
- [ ] 2.3 Implement `renderText` with the precedence `#`, `b`, `B`, `X`, `C`, `P`, `.`,
...
- [ ] 2.4 Verify the round-trip property: render, parse, render again, and confirm the two
      renderings are byte-identical — including for a state with a crate on a pad, which
      exercises the `X` path in both directions.

## 3. Stepping

- [ ] 3.1 Implement `isBlocked(state, col, row)` returning true for a wall or a cell outside
      `0 <= col < 8`, `0 <= row < 6`. Verify it reports blocked for every border cell and
      for coordinates beyond each of the four edges.
- [ ] 3.2 Implement `step` for movement onto floor or an empty pad, updating facing and
...
- [ ] 3.3 Implement blocked movement: turn to face the direction, move nothing, count
      nothing. Verify stepping the initial state left leaves the bot at (1,3) facing left
      with the counter at 0.
- [ ] 3.4 Implement pushing onto floor or an empty pad — crate forward one cell, bot into
...
- [ ] 3.5 Implement blocked pushes for a wall, an out-of-bounds cell, or a second crate
...
- [ ] 3.6 Verify exactly one crate moves per push and that no rule moves a crate toward the
      bot, by asserting crate count and positions across each pushing case above.
- [ ] 3.7 Make `step` throw on a direction outside `U`, `D`, `L`, `R`. Verify the throw, so
      a mistyped move string in a test fails loudly instead of reading as a blocked press.

## 4. Solving and the freeze

- [ ] 4.1 Implement `isSolved` derived from crates and pads, with no stored flag. Verify it
      is false for the initial state and false whenever any crate is off a pad.
- [ ] 4.2 Implement the post-solve freeze as the first branch of `step`, returning the
...
- [ ] 4.3 Verify reference identity holds on the frozen branch, since the shell milestone
      will use `next !== state` as its redraw signal, and verify a blocked press does return
      a new object so that a turn still redraws.

## 5. Replay

- [ ] 5.1 Implement `playMoves` as a fold of `step` over the string's characters. Verify
      replaying `LLL` leaves the bot at (1,3) facing left with the counter at 0.
- [ ] 5.2 Verify the canonical solution: replaying `UURDLDRDRRUURUL` from a freshly parsed
...
- [ ] 5.3 Verify replay cannot count past a solve: append extra directions to the canonical
      solution and confirm the counter still reads 15 and the board is unchanged.

## 6. Immutability, determinism, and restart

- [ ] 6.1 Verify a step does not mutate its argument, by snapshotting the original state's
...
- [ ] 6.2 Verify determinism: replaying the same move string twice from freshly parsed
      states yields identical renderings and identical counters.
- [ ] 6.3 Verify restart equivalence: a state parsed afresh equals a freshly parsed state
      with the counter at 0, asserted both after a solve and partway through a game.

## 7. Milestone verification

- [ ] 7.1 Run `node --test test.js` and verify the whole suite passes with no test skipped.
- [ ] 7.2 Verify every §6 scenario naming a core function has a corresponding test, and that
      ...
      milestone.
- [ ] 7.3 Verify no `package.json` was added and that `game.js` and `test.js` reference no
