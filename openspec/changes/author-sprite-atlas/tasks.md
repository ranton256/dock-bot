...
- [x] 1.1 Add `tools/png.js` with a dependency-free PNG writer: signature, `IHDR`,
...
- [x] 1.2 Verify the writer round-trips through Node's own `zlib.inflateSync`: inflate the
      `IDAT` of the 2×2 image, strip filter bytes, and confirm the recovered pixels match
      the input exactly.

## 2. Independent validation

- [x] 2.1 Add `tools/decode-png.js`: read a PNG from disk, parse chunks, inflate `IDAT`,
...
- [x] 2.2 Add `tools/validate-atlas.js` asserting the mechanical requirements against the
...
- [x] 2.3 Verify the validator actually catches each failure class by running it against
...
- [x] 3.1 Add `tools/atlas.js` defining the named palette — cool steel greys, teal, amber —
...
- [x] 3.2 Implement parsing of a frame as 16 lines of 16 characters into RGBA pixels, and
...
- [x] 4.1 Author `floor`, `wall`, and `pad` as fully opaque 16×16 maps. Verify the
      validator's opacity assertions pass and that `pad` is distinguishable from `floor`
      when the two are viewed side by side at ×3.

## 5. Entity frames

- [x] 5.1 Author the four bot frames with an identical chassis silhouette and a
...
- [x] 5.2 Author `crate` with a transparent background. Verify the transparency assertion
      passes and the crate reads as a crate at ×3.
- [x] 5.3 Author `crate_docked` as the crate plus a glowing edge, with no pad backdrop
...
- [x] 6.1 Add `tools/build-atlas.js` writing `assets/dock_bot.png`, and verify running it
      twice produces byte-identical output.
- [x] 6.2 Run the validator against the generated `assets/dock_bot.png` and verify every
      mechanical requirement passes.
- [x] 6.3 Human review at final size: render all nine frames at ×3 alongside a mock board
...
- [x] 6.4 Verify the shipped game surface is unchanged: `assets/dock_bot.png` is the only
