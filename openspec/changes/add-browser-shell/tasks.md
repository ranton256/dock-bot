...
- [ ] 1.1 Create `index.html` with a UTF-8 charset declaration, a canvas whose `width` and
...
- [ ] 1.2 Create `style.css` covering page and display presentation only. Verify it sets no
...
- [ ] 1.3 Verify the file list is exactly `index.html`, `style.css`, `game.js`, `test.js`,
      and `assets/dock_bot.png` — complete, with nothing added beyond it.

## 2. Shell seam

- [ ] 2.1 Append the shell half to `game.js`, wrapped in a guard on `document` existing.
...
- [ ] 2.2 Verify the core half above the seam is unmodified by this milestone, by diffing it
      against the previous milestone's version.
- [ ] 2.3 Verify the shell contains no rule logic: no cell arithmetic, no push legality
...
- [ ] 3.1 Load the atlas through an `<img>` element and perform the first draw inside its
      ...
      server running.
- [ ] 3.2 Attach the key listener inside the same load handler. Verify that a key pressed
...
- [ ] 4.1 Obtain the 2D context, disable image smoothing on it, and record the nine atlas
...
- [ ] 4.2 Implement `draw(state)` in the specified order: clear the whole canvas, draw every
...
- [ ] 4.3 Draw a crate with the docked frame when its cell is a pad and the plain frame
...
- [ ] 4.4 Select the bot frame from its facing. Verify all four facings by turning the bot
      against a wall in each direction and confirming the drawn frame changes each time.

## 5. Input

- [ ] 5.1 Listen for key events on the window, mapping the four arrow keys to a step and
...
- [ ] 5.2 Discard events whose repeat flag is set. Verify that holding an arrow key down
      produces exactly one move, not a stream of them.
- [ ] 5.3 Bind `R` to re-parsing the level and redrawing unconditionally. Verify it restarts
      from a mid-game state and from a solved one, with the display returning to `Moves: 0`.
- [ ] 5.4 Verify every other key is inert: press letters, digits, modifiers, space, and tab,
      and confirm the board does not change and no redraw occurs.
- [ ] 5.5 Redraw only when the next state is not the same object as the current one. Verify a
      blocked press redraws the turned bot, and that an arrow press after solving redraws
      nothing at all.

## 6. Heads-up display

- [ ] 6.1 Derive the display text from state on every draw, as text content rather than
      markup. Verify it reads `Moves: 0` at boot and tracks the counter as moves are made.
- [ ] 6.2 Show `Solved in N moves — press R` once solved, with the em dash written as an
